...
MEASURED_MDV_FILE = "mid_imputed_what.tsv"
...
res = ifit.solve_with_confidence_intervals(
    solver="ralg",
    ini_fluxes="estimated_net_fluxes_imputed.xlsx",
...
for which, name in [
    ...
    try:
        cis = res.estimate_confidence_intervals(which=which, confidence_level=0.95)
        df = pd.DataFrame(cis, index=["LB", "UB"]).T
        out = f"{name}_mc_CIs_imputed.xlsx"
        df.to_excel(out)
