...
java -jar /home/duford/Finito/mapping/rdt.jar -Q SMI -q "$smiles" -g -c -j AAM -f TEXT
...
species_id_without_cmp=$(grep -F "$(cat ${fn2}.inchikey)" species_id_inchikey.txt | cut -f1 | sed 's/_DASH_/-/g')


if [ -z "$species_id_without_cmp" ]; then
  # optional: log for debugging
  echo "WARN: No species match for $(cat "${fn2}.inchikey") in ${fn2} — skipping." >&2
  counter=$((counter + 1))
  continue
fi
...
species_id=$(grep -F -m 1 "$species_id_without_cmp" from_species_with_cmp | tr -d '\r')

mapping_side="from"
mapping_end="="
else
species_id=$(grep -F -m 1 "$species_id_without_cmp" to_species_with_cmp | tr -d '\r')
mapping_side="to"
mapping_end=","
fi
